...
for(var j=7; j>=0; j--) {
	...
	for(var i=0; i<16; i++) {
	    if(i%2 == 0){
		matrixData += '<td><div class="LEDg" id="id'+i+'_'+j+
		'" onclick="LEDclick('+i+','+j+')">'+
		i+','+j+'</div></td>';
	    }else{
		matrixData += '<td><div class="LEDr" id="id'+i+'_'+j+
		'" onclick="LEDclick('+i+','+j+')">'+
		i+','+j+'</div></td>';
		}
	 }
...
//		}
    ...
    function matrix(data) {
        ...
        for (i = 0; i < data.length; i += 1) {
            disp[i] = parseInt(data[i], 16);//+1]+data[i].replace("0x",""), 16);
        }
        ...
        for (i = 0; i < disp.length; i++) {
			...
			$('#id'+i+'_'+j).addClass('onr');
		    }else if(i%2 == 0){//if($('#id'+i+'_'+j).classList.contains('on')){
			$('#id'+i+'_'+j).removeClass('on');
		    }else{
			$('#id'+i+'_'+j).removeClass('onr');
			}
